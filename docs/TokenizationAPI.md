...
> TokenizationListPermissionsResponse ListTokenizationPermissions(ctx, tokenId).Address(address).Limit(limit).After(after).Before(before).Direction(direction).Execute()
