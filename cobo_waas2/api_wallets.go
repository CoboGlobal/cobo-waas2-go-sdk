...
type ApiListTransactionUtxosRequest struct {
	ctx context.Context
	ApiService *WalletsAPIService
	walletId string
	chainId *string
...
// The chain ID, which is the unique identifier of a blockchain. You can retrieve the IDs of all the chains you can use by calling [List enabled chains](https://www.cobo.com/developers/v2/api-references/wallets/list-enabled-chains).
func (r ApiListTransactionUtxosRequest) ChainId(chainId string) ApiListTransactionUtxosRequest {
	r.chainId = &chainId
	return r
}

// The transaction hash to filter UTXOs by.
func (r ApiListTransactionUtxosRequest) TxHash(txHash string) ApiListTransactionUtxosRequest {
	r.txHash = &txHash
	return r
}

// Filter UTXOs by whether they are change outputs.
func (r ApiListTransactionUtxosRequest) IsChange(isChange bool) ApiListTransactionUtxosRequest {
	r.isChange = &isChange
	return r
}

// The output index of the UTXO to filter by.
func (r ApiListTransactionUtxosRequest) VoutN(voutN int32) ApiListTransactionUtxosRequest {
	r.voutN = &voutN
	return r
}

// The token ID, which is the unique identifier of a token. You can retrieve the IDs of all the tokens you can use by calling [List enabled tokens](https://www.cobo.com/developers/v2/api-references/wallets/list-enabled-tokens).
func (r ApiListTransactionUtxosRequest) TokenId(tokenId string) ApiListTransactionUtxosRequest {
	r.tokenId = &tokenId
	return r
}

func (r ApiListTransactionUtxosRequest) Address(address string) ApiListTransactionUtxosRequest {
	r.address = &address
	return r
}

// The maximum number of objects to return. For most operations, the value range is [1, 50].
func (r ApiListTransactionUtxosRequest) Limit(limit int32) ApiListTransactionUtxosRequest {
	r.limit = &limit
	return r
}

// A cursor indicating the position before the current page. This value is generated by Cobo and returned in the response. If you are paginating forward from the beginning, you do not need to provide it on the first request. When paginating backward (to the previous page), you should pass the before value returned from the last response. 
func (r ApiListTransactionUtxosRequest) Before(before string) ApiListTransactionUtxosRequest {
	r.before = &before
	return r
}

// A cursor indicating the position after the current page. This value is generated by Cobo and returned in the response. You do not need to provide it on the first request. When paginating forward (to the next page), you should pass the after value returned from the last response. 
func (r ApiListTransactionUtxosRequest) After(after string) ApiListTransactionUtxosRequest {
	r.after = &after
	return r
}

func (r ApiListTransactionUtxosRequest) Execute() (*ListUtxos200Response, *http.Response, error) {
	return r.ApiService.ListTransactionUtxosExecute(r)
}

/*
ListTransactionUtxos List transaction UTXOs

The operation retrieves a list of UTXOs for a specified transaction of a wallet. You must specify the wallet, chain, and transaction hash.
...
<Note>This operation is applicable to MPC and Custodial Web3 Wallets.</Note>
 ...
 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param walletId The wallet ID.
 @return ApiListTransactionUtxosRequest
*/
func (a *WalletsAPIService) ListTransactionUtxos(ctx context.Context, walletId string) ApiListTransactionUtxosRequest {
	return ApiListTransactionUtxosRequest{
		ApiService: a,
		ctx: ctx,
		walletId: walletId,
...
func (a *WalletsAPIService) ListTransactionUtxosExecute(r ApiListTransactionUtxosRequest) (*ListUtxos200Response, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "WalletsAPIService.ListTransactionUtxos")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/wallets/{wallet_id}/transaction_utxos"
	localVarPath = strings.Replace(localVarPath, "{"+"wallet_id"+"}", url.PathEscape(parameterValueToString(r.walletId, "walletId")), -1)
	...
	if r.isChange != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "is_change", r.isChange, "")
	}
	if r.voutN != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "vout_n", r.voutN, "")
	}
	if r.tokenId != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "token_id", r.tokenId, "")
	}
