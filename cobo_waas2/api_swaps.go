...
type ApiCreateSwapQuoteRequest struct {
	ctx context.Context
	ApiService *SwapsAPIService
...
// The request body for creating a swap activity.
func (r ApiCreateSwapQuoteRequest) CreateSwapQuoteRequest(createSwapQuoteRequest CreateSwapQuoteRequest) ApiCreateSwapQuoteRequest {
	r.createSwapQuoteRequest = &createSwapQuoteRequest
	return r
}

func (r ApiCreateSwapQuoteRequest) Execute() (*CreateSwapQuote201Response, *http.Response, error) {
	return r.ApiService.CreateSwapQuoteExecute(r)
}

/*
CreateSwapQuote Create Swap Quote

This operation retrieves a quote for swapping between two tokens. Either pay_amount or receive_amount must be provided.


 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @return ApiCreateSwapQuoteRequest
*/
func (a *SwapsAPIService) CreateSwapQuote(ctx context.Context) ApiCreateSwapQuoteRequest {
	return ApiCreateSwapQuoteRequest{
		ApiService: a,
		ctx: ctx,
	}
...
func (a *SwapsAPIService) CreateSwapQuoteExecute(r ApiCreateSwapQuoteRequest) (*CreateSwapQuote201Response, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *CreateSwapQuote201Response
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "SwapsAPIService.CreateSwapQuote")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}
	...
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.createSwapQuoteRequest == nil {
		return localVarReturnValue, nil, reportError("createSwapQuoteRequest is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.createSwapQuoteRequest
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
		return localVarReturnValue, nil, err
	}
...
// The amount of pay token to swap.
