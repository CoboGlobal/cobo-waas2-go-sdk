/*
Cobo Wallet as a Service 2.0

Contact: help@cobo.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package cobo_waas2

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the TronContractCallDestination type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &TronContractCallDestination{}

// TronContractCallDestination The information about the transaction destination. Refer to [Transaction sources and destinations](https://www.cobo.com/developers/v2/guides/transactions/sources-and-destinations) for a detailed introduction about the supported sources and destinations for each transaction type.
type TronContractCallDestination struct {
	DestinationType ContractCallDestinationType `json:"destination_type"`
	// The destination address.
	Address string `json:"address"`
	// The transfer amount. For example, if you trade 1.5 TRX, then the value is `1.5`. 
	Value *string `json:"value,omitempty"`
	// The data that is used to invoke a specific function or method within the specified contract at the destination address. 
	Calldata string `json:"calldata"`
}

type _TronContractCallDestination TronContractCallDestination

// NewTronContractCallDestination instantiates a new TronContractCallDestination object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewTronContractCallDestination(destinationType ContractCallDestinationType, address string, calldata string) *TronContractCallDestination {
	this := TronContractCallDestination{}
	this.DestinationType = destinationType
	this.Address = address
	this.Calldata = calldata
	return &this
}

// NewTronContractCallDestinationWithDefaults instantiates a new TronContractCallDestination object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewTronContractCallDestinationWithDefaults() *TronContractCallDestination {
	this := TronContractCallDestination{}
	return &this
}

// GetDestinationType returns the DestinationType field value
func (o *TronContractCallDestination) GetDestinationType() ContractCallDestinationType {
	if o == nil {
		var ret ContractCallDestinationType
		return ret
	}

	return o.DestinationType
}

// GetDestinationTypeOk returns a tuple with the DestinationType field value
// and a boolean to check if the value has been set.
func (o *TronContractCallDestination) GetDestinationTypeOk() (*ContractCallDestinationType, bool) {
	if o == nil {
		return nil, false
	}
	return &o.DestinationType, true
}

// SetDestinationType sets field value
func (o *TronContractCallDestination) SetDestinationType(v ContractCallDestinationType) {
	o.DestinationType = v
}

// GetAddress returns the Address field value
func (o *TronContractCallDestination) GetAddress() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Address
}

// GetAddressOk returns a tuple with the Address field value
// and a boolean to check if the value has been set.
func (o *TronContractCallDestination) GetAddressOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Address, true
}

// SetAddress sets field value
func (o *TronContractCallDestination) SetAddress(v string) {
	o.Address = v
}

// GetValue returns the Value field value if set, zero value otherwise.
func (o *TronContractCallDestination) GetValue() string {
	if o == nil || IsNil(o.Value) {
		var ret string
		return ret
	}
	return *o.Value
}

// GetValueOk returns a tuple with the Value field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TronContractCallDestination) GetValueOk() (*string, bool) {
	if o == nil || IsNil(o.Value) {
		return nil, false
	}
	return o.Value, true
}

// HasValue returns a boolean if a field has been set.
func (o *TronContractCallDestination) HasValue() bool {
	if o != nil && !IsNil(o.Value) {
		return true
	}

	return false
}

// SetValue gets a reference to the given string and assigns it to the Value field.
func (o *TronContractCallDestination) SetValue(v string) {
	o.Value = &v
}

// GetCalldata returns the Calldata field value
func (o *TronContractCallDestination) GetCalldata() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Calldata
}

// GetCalldataOk returns a tuple with the Calldata field value
// and a boolean to check if the value has been set.
func (o *TronContractCallDestination) GetCalldataOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Calldata, true
}

// SetCalldata sets field value
func (o *TronContractCallDestination) SetCalldata(v string) {
	o.Calldata = v
}

func (o TronContractCallDestination) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o TronContractCallDestination) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["destination_type"] = o.DestinationType
	toSerialize["address"] = o.Address
	if !IsNil(o.Value) {
		toSerialize["value"] = o.Value
	}
	toSerialize["calldata"] = o.Calldata
	return toSerialize, nil
}

func (o *TronContractCallDestination) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"destination_type",
		"address",
		"calldata",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varTronContractCallDestination := _TronContractCallDestination{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	//decoder.DisallowUnknownFields()
	err = decoder.Decode(&varTronContractCallDestination)

	if err != nil {
		return err
	}

	*o = TronContractCallDestination(varTronContractCallDestination)

	return err
}

type NullableTronContractCallDestination struct {
	value *TronContractCallDestination
	isSet bool
}

func (v NullableTronContractCallDestination) Get() *TronContractCallDestination {
	return v.value
}

func (v *NullableTronContractCallDestination) Set(val *TronContractCallDestination) {
	v.value = val
	v.isSet = true
}

func (v NullableTronContractCallDestination) IsSet() bool {
	return v.isSet
}

func (v *NullableTronContractCallDestination) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableTronContractCallDestination(val *TronContractCallDestination) *NullableTronContractCallDestination {
	return &NullableTronContractCallDestination{value: val, isSet: true}
}

func (v NullableTronContractCallDestination) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableTronContractCallDestination) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


