...
// KytScreeningsDecisionsType The KYT screening decision type for a specific transaction based on an external compliance review.  - `Approval`: Transaction is approved and can proceed without restrictions - `ApprovalWithAlert`: Transaction is approved but flagged for monitoring or additional compliance attention - `Rejection`: Transaction is rejected due to identified compliance risks and should be blocked - `ManualReview`: Transaction requires manual review by compliance officers before final determination 
