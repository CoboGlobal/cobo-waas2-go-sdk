...
import (
    ...
    "github.com/CoboGlobal/cobo-portal-go-api/crypto"
)
...
func (c *APIClient) prepareRequest(
	...
	if ctx != nil {
		...
		// Portal Signature Authentication
		if signer, ok := ctx.Value(ContextPortalSigner).(crypto.ApiSigner); ok {
			...
			hash, err := crypto.GetUnsignedDigest(localVarRequest, reader, timestamp)
			if err != nil {
				return nil, err
			}

			signature := signer.Sign(hash)
			localVarRequest.Header.Add("Biz-Api-Key", signer.GetPublicKey())
			localVarRequest.Header.Add("Biz-Api-Nonce", timestamp)
			localVarRequest.Header.Add("Biz-Api-Signature", signature)
		}
	}
